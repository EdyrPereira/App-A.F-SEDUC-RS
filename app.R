...
server <- function(input, output, session) {
  ...
  # Exportação em TXT: Relatório Resumido do Cenário
  ...
  output$baixar_relatorio_txt <- downloadHandler(
    ...
    content = function(file) {
      req(tb_valores_base_react(), tb_impacto_react(), tb_fixo_react(), 
          tb_variavel_react(), tb_equidade_react())
      
      # Função auxiliar para formatar tabelas dentro do TXT
      fmt_tab <- function(df) {
        paste(capture.output(print(as.data.frame(df), row.names = FALSE)), collapse = "\n")
      }
      
      # Função auxiliar para formatar tabelas em texto sem quebrar colunas
      fmt_tab <- function(df) {
        # Guarda a largura original e define temporariamente para 1000 caracteres
        largura_antiga <- getOption("width")
        options(width = 10000)
        on.exit(options(width = largura_antiga))
        
        # Converte a tabela para texto em uma única linha contínua
        paste(capture.output(print(as.data.frame(df), row.names = FALSE)), collapse = "\n")
      }
      ...
      conteudo <- c(
        ...
        fmt_tab(tb_valores_base_react()),
        "",
        "--------------------------------------------------------------------------------",
        "--- 2. RESUMO DE IMPACTO DA REDE ---",
        fmt_tab(tb_impacto_react()),
        "",
        "--------------------------------------------------------------------------------",
        "--- 3. RESUMO DA PARCELA FIXA ---",
        fmt_tab(tb_fixo_react()),
        "",
        "--------------------------------------------------------------------------------",
        "--- 4. RESUMO DA PARCELA VARIÁVEL ---",
        fmt_tab(tb_variavel_react()),
        "",
        "--------------------------------------------------------------------------------",
        "--- 5. RESUMO DE EQUIDADE ---",
        fmt_tab(tb_equidade_react()),
        "",
        "================================================================================",
        " FIM DO RELATÓRIO"
      )
