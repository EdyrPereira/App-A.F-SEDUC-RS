...
downloadButton <- function(outputId, label = "Download", class = NULL, ...) {
  # Cria o botão padrão do Shiny
  tag <- shiny::downloadButton(outputId, label, class = class, ...)
  
  # Remove o atributo que o Chrome bloqueia no Shinylive
  tag$attribs$download <- NULL
  
  # Injeta um pequeno script em JavaScript que força o Chrome a baixar como arquivo UTF-8
  tag$id <- outputId
  script <- tags$script(HTML(sprintf("
    document.getElementById('%s').addEventListener('click', function(e) {
      // Aguarda o Shiny gerar o link interno
      setTimeout(() => {
        let href = this.getAttribute('href');
        if (href && !href.startsWith('blob:')) {
          // Busca o conteúdo gerado pelo Shinylive
          fetch(href).then(res => res.blob()).then(blob => {
            // Força a codificação UTF-8 no arquivo gerado
            let novoBlob = new Blob([blob], { type: 'text/csv;charset=utf-8;' });
            let url = URL.createObjectURL(novoBlob);
            
            // Cria um link temporário oculto para disparar o download real
            let a = document.createElement('a');
            a.href = url;
            a.download = '%s.csv'; // Nome padrão caso falte
