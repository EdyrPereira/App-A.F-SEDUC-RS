...
server <- function(input, output, session) {
  ...
  # Teste rápido para validar o gerador de Excel
  output$baixar_relatorio_completo <- downloadHandler(
    filename = function() {
      "teste_relatorio.xlsx"
    },
    content = function(file) {
      # Cria um Excel simples com dados fictícios para testar a gravação
      dados_teste <- list(
        "Teste" = data.frame(Status = "O writexl esta funcionando no Shinylive!", Data = Sys.Date())
      )
      writexl::write_xlsx(dados_teste, path = file)
    }
  )
}
