...
server <- function(input, output, session) {
  ...
  output$baixar_relatorio_completo <- downloadHandler(
    ...
    content = function(file) {
      # 1. Garante que nenhuma tabela reativa esteja nula/vazia ao clicar
      req(dados_simulados(), tb_valores_base_react(), tb_impacto_react(), 
          tb_fixo_react(), tb_variavel_react(), tb_equidade_react())
      
      # 2. Prepara a base de escolas limpando aspas de forma ultra segura
      df_escolas <- as.data.frame(dados_simulados()$dados)
      cols_texto <- sapply(df_escolas, is.character)
      df_escolas[cols_texto] <- lapply(df_escolas[cols_texto], function(x) gsub('"', '', x))
      
      # 3. Garante que TODAS as abas sejam estritamente data.frames puros
      lista_de_abas <- list(
        "Escolas Simuladas"     = df_escolas,
        "Orçamentos e Vlr Base" = as.data.frame(tb_valores_base_react()),
        "Resumo de Impacto"     = as.data.frame(tb_impacto_react()),
        "Resumo Parcela Fixa"   = as.data.frame(tb_fixo_react()),
        "Resumo Parcela Var"    = as.data.frame(tb_variavel_react()),
        "Resumo Equidade"       = as.data.frame(tb_equidade_react())
      )
      
      # 4. Escreve o arquivo .xlsx
